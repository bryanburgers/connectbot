...
impl From<control::ClientsResponse_Client> for Device {
    fn from(mut client: control::ClientsResponse_Client) -> Self {
        ...
        let connection_history = client
            .take_connection_history()
            .into_iter()
            .map(Into::into)
            .collect();
        ...
        let address = if address == "" { None } else { Some(address) };
...
impl From<control::ClientsResponse_ConnectionHistoryItem> for DeviceHistoryItem {
    fn from(client: control::ClientsResponse_ConnectionHistoryItem) -> Self {
        ...
        match history_type {
            ...
            control::ClientsResponse_ConnectionHistoryType::CLOSED => DeviceHistoryItem {
                connected_at: connected_at.to_rfc3339(),
                last_message: Some(last_message.to_rfc3339()),
                address,
            },
            control::ClientsResponse_ConnectionHistoryType::OPEN => DeviceHistoryItem {
                connected_at: connected_at.to_rfc3339(),
                last_message: None,
                address,
            },
...
impl From<control::ClientsResponse_Connection> for DeviceConnection {
    fn from(connection: control::ClientsResponse_Connection) -> Self {
        let active_until = if connection.get_active_until() == 0 {
            ...
            Some(
                Utc.timestamp(connection.get_active_until() as i64, 0)
                    .to_rfc3339(),
            )
        };
        ...
        DeviceConnection {
            ...
            client_state: match connection.get_state() {
                control::ClientsResponse_ClientState::UNKNOWN_STATE => {
                    DeviceConnectionClientState::Failed
                }
                control::ClientsResponse_ClientState::REQUESTED => {
                    DeviceConnectionClientState::Requested
                }
                control::ClientsResponse_ClientState::CONNECTING => {
                    DeviceConnectionClientState::Connecting
                }
                control::ClientsResponse_ClientState::CONNECTED => {
                    DeviceConnectionClientState::Connected
                }
                control::ClientsResponse_ClientState::DISCONNECTING => {
                    DeviceConnectionClientState::Disconnecting
                }
                control::ClientsResponse_ClientState::DISCONNECTED => {
                    DeviceConnectionClientState::Disconnected
                }
                control::ClientsResponse_ClientState::FAILED => DeviceConnectionClientState::Failed,
            },
            active_state: match connection.get_active() {
                control::ClientsResponse_ActiveState::UNKNOWN_ACTIVE_STATE => {
                    DeviceConnectionActiveState::Inactive
                }
